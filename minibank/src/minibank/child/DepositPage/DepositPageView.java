...
public class DepositPageView extends JFrame {
    ...
    private void initializeUI() {
        ...
        // 底层图片背景
        ImageIcon imageIcon = new ImageIcon(getClass().getResource("/DepositPage.png"));
        ...
        JButton buttonDeposit = createButton("deposit", 245, 535, 110, 50);
...
//        Color lightBlue = new Color(217, 228, 239);
//        inputTextField.setBackground(lightBlue);
//        inputTextField.setEditable(false);
//        add(inputTextField);

        // 创建最下方提示取钱的JLabel以及sure/not按钮  //(尚未完成，需要实现点击buttonDeposit按钮才显示下方的提示文本以及输入的内容和按钮，文本deposit后边显示存钱数额的变量)
//        JLabel promptlabel = new JLabel("Your want to deposit 这里为存钱金额变量, are you sure?");
...
//        JButton buttonNot = createButton("Not", 730, 710, 80, 30);    //点击后回到显示提示存钱确定与否之前
...
//        layeredPane.add(buttonSure, Integer.valueOf(1));
    ...
    private void acknowledgeWindow(int deposit){
        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame();
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.setSize(600, 400);
            frame.setLocationRelativeTo(null);
            showDepositDialog(frame,deposit,"deposit");
